# using simulated annealing to solve optimization problems
# this is better than the simple hill climbing method, where the global optimum was quite hard to find
import numpy as np
import random

N = 100         # size of the problem is N x N
steps = 3000    # total number of iterations
tracks = 50     # number of agents finding the solution
...
# generate values for each xy coordinate
h = np.fromfunction(np.vectorize(generator), (N, N), x0=x0, y0=y0, dtype=int)
peak_x, peak_y = np.unravel_index(np.argmax(h), h.shape)

# starting points stored in lists of 50
    ...
    for step in range(steps):
        ...
        for i in range(tracks):
            ...
            if S_new > S_old:
                ...
                if T == 0:
                    continue                # if the new solution is worse, do nothing
